...
export default memo(function SongDetailLeft() {
  ...
  const collectSonglist = useCallback(() => {
    if(isLogin) {
      // Favorite Song List Interface
      sendCollectSonglist(songDetail.id, cookie).then((res) => {
        if (res.code === 200) message.success('Collect Successful')
        console.log('res :>>>', res)
      })
    } else {
      ...
      <HeaderTitle className="flex">
        ...
        <div className="song-detail">
          ...
          <div className="avatar">
            ...
            <div className="avatar-datetime">{avatarDatetime}Create</div>
          </div>
          ...
          <div className="description-info gap">
            <div className="descript-detail">Introduction: {description}</div>
          </div>
        </div>
      </HeaderTitle>
      {/* Song List Details Music List */}
      <MainDetail>
        <ThemeHeaderRcm
          className="gap"
          title="song list"
